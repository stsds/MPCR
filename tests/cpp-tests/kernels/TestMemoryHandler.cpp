...
TEST_MEMORY_HANDLER_CUDA() {
    ...
    SECTION("Device Allocation"){
        auto *pdata_alloc_device = memory::AllocateArray(sizeof(float) * n, GPU, &context);
        REQUIRE(pdata_alloc_device != nullptr);

        REQUIRE_THROWS(memory::MemCpy(pdata_alloc_device, (char *) pdata_host, sizeof(float) * n, &context,
                                      memory::MemoryTransfer::HOST_TO_DEVICE));

        context.SetOperationPlacement(GPU);
        context.SetRunMode(kernels::RunMode::SYNC);

        memory::MemCpy(pdata_alloc_device, (char *) pdata_host, sizeof(float) * n, &context,
                       memory::MemoryTransfer::HOST_TO_DEVICE);

        auto *pdata_alloc_host=memory::AllocateArray(sizeof (float)*n,CPU,&context);
        ...
        memory::MemCpy(pdata_alloc_host,pdata_alloc_device ,sizeof(float) * n, &context,
        ...
        REQUIRE_THROWS(memory::Memset(pdata_alloc_device,'c',n,GPU,&context));

        context.SetOperationPlacement(GPU);
        memory::Memset(pdata_alloc_device,'c',n,GPU,&context);

        auto *pdata_alloc_device_two = memory::AllocateArray(sizeof(float) * n, GPU, &context);
        REQUIRE(pdata_alloc_device_two!= nullptr);

        memory::MemCpy(pdata_alloc_device_two,pdata_alloc_device ,sizeof (float)*n, &context,
                       memory::MemoryTransfer::DEVICE_TO_DEVICE);

        memory::MemCpy(pdata_alloc_host,pdata_alloc_device_two ,sizeof (float)*n, &context,
                       memory::MemoryTransfer::DEVICE_TO_HOST);
...
TEST_MEMORY_HANDLER(){
    ...
    SECTION("Host Allocation"){
        ...
        memory::MemCpy(pdata_alloc_host, (char *) pdata_host, sizeof(float) * n, &context,
