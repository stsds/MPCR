...
TEST_CUDA_STREAMS() {
    ...
    SECTION("Test Synchronous GPU context") {
        ...
        auto newContext = ContextManager.CreateRunContext();
        ...
        auto newContextCPU = ContextManager.CreateRunContext();
        newContext->SetOperationPlacement(CPU);  // Set the new context to use GPU
        ContextManager.SetOperationContext(newContextCPU);
        mpcr::kernels::ContextManager::GetOperationContext()->SetOperationPlacement(
                CPU);

        auto data = (float *) output.GetData();
        auto data_validate = (float *) output_validate.GetData();
        ...
        for (int i = 0; i < output.GetSize(); ++i) {
            ...
            std::cout << data_validate[i] << std::endl;
...
//        b.ToMatrix(size, size);
//
//        DataType output_validate_Async(DOUBLE);
//        DataType output_validate_Sync(DOUBLE);
//        DataType outputSync(DOUBLE);
//        DataType outputAsync(DOUBLE);
//
//        SIMPLE_DISPATCH(DOUBLE, linear::CrossProduct, a, b, output_validate_Async, false, false)
//        SIMPLE_DISPATCH(DOUBLE, linear::CrossProduct, a, output_validate_Async, output_validate_Sync, false, false)
//
//        auto& context = mpcr::kernels::ContextManager::GetInstance();
//        auto newAyncContext = context.CreateRunContext();
//        newAyncContext->SetOperationPlacement(GPU);  // Set the new context to use GPU
...
//        newSyncContext->SetRunMode( RunMode::SYNC);   // Set to synchronous mode
//
//        SIMPLE_DISPATCH(DOUBLE, linear::CrossProduct, a, outputAsync, outputSync, false,false)
//
//        REQUIRE(outputSync.GetNRow() == size);
//        REQUIRE(outputSync.GetNCol() == size);
//
...
//
//        for (int i = 0; i < outputSync.GetSize(); ++i) {
//            double val = fabs(outputSync.GetVal(i) - output_validate_Sync.GetVal(i)) / output_validate_Sync.GetVal(i);
//            REQUIRE(val <= error_threshold);
//        }
//        context.SyncContext(1);
//        for (int i = 0; i < outputAsync.GetSize(); ++i) {
//            double val = fabs(outputAsync.GetVal(i) - output_validate_Async.GetVal(i)) / output_validate_Async.GetVal(i);
//            REQUIRE(val <= error_threshold);
//        }
//
//        REQUIRE(outputAsync.GetNRow() == size);
