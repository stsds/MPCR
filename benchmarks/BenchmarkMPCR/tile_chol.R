...
run_chol_benchmark <- function(n, tile_size, replication, times, num_threads) {
  ...
  MPCR_tile_matrix_single <- new(MPCRTile, n, n, tile_size, tile_size, matrix, precision_single)
  MPCR_tile_matrix_double <- new(MPCRTile, n, n, tile_size, tile_size, matrix, precision_double)


  cat("\n\n\n")
  cat("Running tile chol benchmark \n")
  print(benchmark(replications = rep(replication, times),
                  chol(MPCR_tile_matrix_single, overwrite_input = FALSE, num_threads = num_threads),
                  chol(MPCR_tile_matrix_double, overwrite_input = FALSE, num_threads = num_threads),
                  columns = c("test", "replications", "elapsed")))

  cat("\n\n\n")
  cat("Running tile chol benchmark with input overwrite \n")
  print(benchmark(replications = rep(1, 1),
                  chol(MPCR_tile_matrix_single, overwrite_input = TRUE, num_threads = num_threads),
                  chol(MPCR_tile_matrix_double, overwrite_input = TRUE, num_threads = num_threads),
                  columns = c("test", "replications", "elapsed")))
