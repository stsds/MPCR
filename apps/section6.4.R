...
run_inla <- function(prec='R-Double', D, y, n) {
	...
	calc_Q <- function(alpha) {
		...
		if(use_mpcr) cov <- as.MPCR(cov, n, n, p, placement=op_place)
	...
	calc_x0 <- function(alpha, tol=1e-12) {
		...
		repeat {
			...
			if(use_mpcr) {
				diag_g <- as.MPCR(as.matrix(diag_g), n, n, p, placement=op_place)
				mode <- as.MPCR(g1-x0*g2, n, 1, p, placement=op_place)
				MPCR.SetOperationPlacement(placement = op_place)
	...
	calc_lpost <- function(alpha) {
		...
		if(use_mpcr) diag_term <- as.MPCR(diag_term, n, n, p, placement=op_place)
		MPCR.SetOperationPlacement(placement = op_place)
		H <- Q + diag_term
		...
		if(use_mpcr) {
			...
			x0_mpcr <- as.MPCR(x0, n, 1, p, placement=op_place)
			MPCR.SetOperationPlacement(placement = op_place)
...
matplot(sizes, timing_results, type="b", lty=1:5, pch=c(21,22,24), 
	col=1:5, lwd=2, log="y",
